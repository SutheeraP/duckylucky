...
const ModalCard = (props:any) => {
    ...
    return(
        <>
            {/* ถ้าถือการ์ดอยู่ แต่ไม่มีการ์ด (โดนลบอันที่ถือ) => reset */}
            {(!(selectedCard === ``) && !inhandCard[selectedCard]) ? resetSelectedCard() : null}
            
            {inhandCard[selectedCard] ?
                <div className="flex flex-row border border-black rounded-lg px-6 py-4 gap-6 relative">
                    {(!(selectedCard === ``) && (inhandCard[selectedCard])) ?
                        <Image src={inhandCard[selectedCard].img} alt="" width={130} height={280} className="border border-black rounded-lg"></Image>
                        : ''}
